    ...
    else
    {
        switch (StringToInt(sEvent))
        {
            ...
            case EVENT_SCRIPT_PLACEABLE_ON_USED:
                SimpleDialog_StartConversation(GetLastUsedBy(), OBJECT_SELF, MUSICPLAYER_SYSTEM_TAG, MusicPlayer_GetIsDisabled() ? 3 : 1);
                break;

            case EVENT_SCRIPT_PLACEABLE_ON_MELEEATTACKED:
            case EVENT_SCRIPT_PLACEABLE_ON_SPELLCASTAT:
                SimpleDialog_DisableMusicPlayer();
                break;
        }
    }
}
...
void MusicPlayer_LoadMusicTracks()
{
    ...
    ES_Util_Log(MUSICPLAYER_SYSTEM_TAG, "* Loaded '" + IntToString(nNumTracks) + "' Music Tracks");
}

void MusicPlayer_CreateConversation()
{
    object oConversation = SimpleDialog_CreateConversation(MUSICPLAYER_SYSTEM_TAG);
...
void MusicPlayer_SpawnPlaceables(string sSubsystemScript)
{
    ...
    pd.sTag = MUSICPLAYER_SYSTEM_TAG + "_PLC";
    pd.bPlot = TRUE;
    pd.bUseable = TRUE;
...
void SimpleDialog_ApplyDisabledEffects(object oMusicPlayer)
{
    if (MusicPlayer_GetIsDisabled(oMusicPlayer))
    {
        ...
        DelayCommand(fDelay, SimpleDialog_ApplyDisabledEffects(oMusicPlayer));
    }
}
